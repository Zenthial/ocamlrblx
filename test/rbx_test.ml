...
let%test "render func_call" =
  let expr = FuncCall { ident = "print"; cparameters = [ Literal (String "test") ] } in
  let result = render expr in
...
let%test "render array" =
  let arr =
    Array { array_members = [ Literal (String "test"); Literal (String "test2") ] }
  in
...
let%test "render map" =
  let map =
    Map { map_members = [ "test", Literal (Number "1"); "test2", Literal (String "1") ] }
  in
