...
let rec render_expression expression =
  ...
  | Ast.Assignment a ->
      sprintf "local %s = %s" a.aname (render_expression a.value)
  | Ast.Func f -> render_function f
  | Ast.BinExp (left, op, right) ->
      sprintf "%s %s %s" (render_expression left) (render_bin_op op)
        (render_expression right)
  | Ast.Identifier i -> i
  | Ast.FuncCall c -> render_func_call c
  | _ -> "unimplemented"
...
and render_function fn_decl =
  ...
  if List.length fn_decl.statements = 0 then header ^ " end"
    ...
    let block =
      List.map (fun s -> "  " ^ render_expression s ^ "\n") fn_decl.statements
      |> String.concat ""
    in
    header ^ "\n" ^ block ^ "end"

and render_call_parameters params =
  List.map render_expression params |> String.concat ", "

and render_func_call (c : Ast.func_call) =
  sprintf "%s(%s)" c.ident (render_call_parameters c.cparameters)
